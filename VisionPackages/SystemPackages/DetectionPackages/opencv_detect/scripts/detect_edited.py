...
class Detector():
    def __init__(self):
        ...
        self.sub = rospy.Subscriber("/astra/cpp_image_node/image_roi", Image, self.colordetect_cb)
        ...
        self.roi = rospy.get_param('/image_viewer_capture/roi')
    ...
    def pcl_cb(self, ros_point_cloud):
        ...
        self.center_point = centerori

        indexinPC = HSVdetect.ismember(np_source, center_point, axis='row')
        centerinsource = np_source[indexinPC]
        ...
        IfTarget = self.colordetect(self.cv_image, sel)
    ...
    def colordetect(self, cv_image, inputpixelidx, radius):
       
        flag = 0
        inputpixelidx = np.array([230,300])
        ...
        for row in range(HeightS):    #遍历每一行
        ...
        point_color = (0, 0, 255)
        thickness = 4 
        cv2.circle(image, [inputpixelidx[1],inputpixelidx[0]], radius, point_color, thickness)
        cv2.imshow('rr',image)
