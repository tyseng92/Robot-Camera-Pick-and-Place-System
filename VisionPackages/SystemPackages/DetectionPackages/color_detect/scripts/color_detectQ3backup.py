...
class Detector():
    ...
    def pcl_cb(self, ros_cloud):
        ...
        heightros = pc.shape[0]
        widthros = pc.shape[1]
        np_points = np.zeros((heightros*widthros, 3),dtype = np.float32)
        np_points[:,0] = np.resize(pc['x'], heightros*widthros)
        np_points[:,1] = np.resize(pc['y'], heightros*widthros)
        np_points[:,2] = np.resize(pc['z'], heightros*widthros)
        np.savetxt('/home/rrc2/catkin_ws/src/color_detect/np_points.txt', np_points, delimiter=',')
        np.savetxt('/home/rrc2/catkin_ws/src/color_detect/pcx.txt', pc['x'], delimiter=',')

        pcdros =o3d.geometry.PointCloud()
        pcdros.points = o3d.utility.Vector3dVector(np_points)



        print('ros_pointcloud:  ', pc)
     ...
     #   print("point cloud width:  ", np_cloud)
       # print('tuple'"start pcl processing", len(np_cloud))
        ...
        print("heightGridbefore:  ",heightGrid)
